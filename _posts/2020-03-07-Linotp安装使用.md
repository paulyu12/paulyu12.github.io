...
5. 安装MySQL 5.7
    ...
    由于CentOS 8使用```yum install mysql-server```命令默认安装的版本是MySQL 7，会在后续步骤中导致未能解决的问题，这里手动安全MySQL 5.7。
...
14. 上面的命令启动LinOTP服务会报错，按照如下步骤处理报错
    ...
    ```
    注意：上面第二条命令将/etc/ssl/private/linotpserver.key拷贝到linotp的配置目录/etc/linotp2/private.pem，但是 “**前万不要**” 执行下面这句：
    ```bash
...
15. 重启LinOTP服务，只剩一个警告
    ...
    请求页面http://localhost:5001/manage，终于出现了下面的页面：
